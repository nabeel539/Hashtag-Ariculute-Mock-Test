...
function TestSetsPage() {
  ...
  useEffect(() => {
    const fetchTestSets = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/test/${id}/sets`, {
        ...
        if (response.data.success) {
          ...
          toast.error("Failed to load test sets");
          navigate("/dashboard/user");
        }
      } catch (error) {
        toast.error("Error fetching test sets");
        navigate("/dashboard/user");
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div>Loading test sets...</div>;
        ...
        <div className="grid gap-6">
          {test.sets.map((set) => (
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-center">
                  <h3 className="font-bold text-xl">{set.title}</h3>
                  {set.attempted && (
              ...
              <div className="p-6 border-t bg-gray-50 flex justify-between items-center">
                ...
                <Button onClick={() => handleSelectSet(set.setId)}>
                  {set.attempted ? "Retake" : "Start"}{" "}
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </div>
